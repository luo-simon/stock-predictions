...
class BaseCLI(LightningCLI):
    ...
    def add_arguments_to_parser(self, parser):
        ...
        parser.add_argument("--n_trials", type=int, help="Number of trials to run for tuning", default=0)
    ...
    def tune(self, model, datamodule):
        ...
        assert len(study.trials) > 0, "No trials found. Run trials first by setting n_trials >= 1."
        best = study.best_trial

        print(f"Best trial was trial number {best.number} with validation loss of {best.value}. Run completed at {best.datetime_complete}")
